...
def generate_mq_spss_syntax(rule):
    ...
    # *******************************************************************
    # *** THIS IS THE LINE THAT WAS REPORTED AS ERROR - CONFIRMED CORRECT ***
    # *******************************************************************
...
def generate_master_spss_syntax(sq_rules, mq_rules, ranking_rules, string_rules):
    ...
    for flag in unique_flag_names:
        ...
        # Any flag starting with 'xx' that is NOT one of the simple data checks and NOT a count
    ...
    if st.session_state.get('sq_batch_vars'):
        ...
        with st.form(sq_config_form_key):
            ...
            for i, col in enumerate(st.session_state.sq_batch_vars):
                ...
                if run_piping:
                    with st.container(border=True):
                        st.warning("Piping check will automatically apply EOO/EOC logic to this variable (`xxQx_i=1/2`), overriding the standard skip logic check (C). A filter flag (`Flag_Qx_BASE`) is required (from Section C).")
    ...
    with st.expander("➕ Add Multi-Select Group Rule", expanded=False):
        ...
        if mq_cols:
            ...
            with st.form(f"mq_form_{mq_set_name}"):
                ...
                run_skip = st.checkbox(f"Enable Skip Logic Check (Creates Flag_Qx and xxQx=1/2)", key=f'mq_run_skip_{mq_set_name}')
                skip_trigger_col = '-- Select Variable --'
                skip_trigger_val = '1'
                if run_skip:
                    with st.container(border=True):
                        ...
                        col_t_col, col_t_val = st.columns(2)
                        with col_t_col:
                            skip_trigger_col = st.selectbox("**Filter/Trigger Variable** (e.g., Q_Prev)", all_variable_options, 
                                                            index=all_variable_options.index('-- Select Variable --'), key=f'mq_t_col_{mq_set_name}')
                        with col_t_val:
