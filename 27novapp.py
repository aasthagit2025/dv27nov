...
# Initialize state (Kept exactly as original)
...
# --- SYNTAX GENERATORS (Preserving core skip logic) ---
def generate_skip_spss_syntax(target_col, trigger_col, trigger_val, rule_type, range_min=None, range_max=None):
    ...
    if rule_type == 'String':
        eoo_cond = f"({target_col}='' | miss({target_col}))"
        eoc_cond = f"({target_col}<>'' & ~miss({target_col}))"
    else:
        eoo_cond = f"miss({target_col})"
        eoc_cond = f"~miss({target_col})"

    syntax.append(f"IF({filter_flag} = 1 & {eoo_cond}) {final_error_flag}=1.")
    syntax.append(f"IF(({filter_flag} <> 1 | miss({filter_flag})) & {eoc_cond}) {final_error_flag}=2.")
    ...
    if st.session_state.get('string_batch_vars'):
        with st.form("string_form"):
            ...
            for i, col in enumerate(st.session_state.string_batch_vars):
                ...
                # ADDED: Skip Logic Filter (Exactly like Single Select UI)
                st.markdown("#### B. Skip Logic Filter Condition")
                c1, c2 = st.columns(2)
                with c1:
...
# --- FINAL GENERATOR (NO SUM + ADDED FREQUENCIES) ---
def generate_master_spss_syntax(sq_rules, mq_rules, string_rules):
    all_syntax = ["DATASET ACTIVATE ALL."]
    all_flags = []

    # Process String Rules with Skip Logic
    ...
    # Compile Frequencies at end for all newly created variables
...
# (Remainder of Original App Logic follows same order)